...
namespace BmxStreetsUI.Components
{
    internal class ModMenu
    {
        ...
        public static void ModMenuSetup(GameObject settingsTab)
        {
            ...
            var ModMenuTab = API.CreateTab("Mods");
        ...
        public static void CharacterMenuSetup()
        {
            ...
            //var characterTab = GameObject.FindObjectsOfType<EventTrigger>( true ).Where((EventTrigger trig) => { return trig.gameObject.name.Contains("CHARACTER - TAB"); });
            ...
            foreach (var configPanel in GameObject.FindObjectsOfType<DataConfigPanel>(true).Where(dataconfig => dataconfig.gameObject.name.Contains(Constants.CharacterSelectionPanel)))
            {
                ...
                menu.OnEnterOpen.RemoveListener(new System.Action(() => SetCharacterButtonTriggers(configPanel, mg)));
                menu.OnEnterOpen.AddListener(new System.Action(() => SetCharacterButtonTriggers(configPanel, mg)));
                SetCharacterButtonTriggers(configPanel, mg);
        ...
        static void SetCharacterButtonTriggers(DataConfigPanel configPanel,MGMenu mg)
        {
            foreach (var trigger in configPanel.gameObject.GetComponentsInChildren<EventTrigger>(true))
            {
